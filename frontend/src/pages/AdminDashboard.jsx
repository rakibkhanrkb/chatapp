...
export default function AdminDashboard() {
  ...
  async function fetchUsers() {
    setLoading(true);
    try {
      const { data } = await axios.get('http://localhost:3001/api/admin/users', { headers: { 'x-user-id': currentUser.id } });
      setUsers(data);
    } catch (err) {
  ...
  async function fetchAllMessages() {
    setLoading(true);
    try {
      const { data } = await axios.get('http://localhost:3001/api/admin/all-messages', { headers: { 'x-user-id': currentUser.id } });
      setAllMessages(data);
    } catch (err) {
      console.error(err);
    } finally { setLoading(false); }
  ...
  async function handleUpdateUser(e) {
    ...
    try {
      await axios.put(`http://localhost:3001/api/admin/users/${editingUser.id}`, editForm, { headers: { 'x-user-id': currentUser.id } });
  ...
  async function deleteUser(id, name) {
    ...
    try {
      await axios.delete(`http://localhost:3001/api/admin/users/${id}`, { headers: { 'x-user-id': currentUser.id } });
  ...
  async function deleteGlobalMessage(id) {
    ...
    try {
      await axios.delete(`http://localhost:3001/api/admin/messages/${id}`, { headers: { 'x-user-id': currentUser.id } });
